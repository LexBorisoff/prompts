...
const getValue = (arr, i) => arr[i] && (arr[i].value || arr[i].title || arr[i]);
...
class AutocompletePrompt extends Prompt {
  ...
  moveSelect(i) {
    this.select = i;
    if (this.suggestions.length > 0) this.value = getValue(this.suggestions, i);
  ...
  async complete(cb) {
    ...
    const currentHighlight = this.suggestions[this.select];
    ...
    // match input exactly
    let newHighlightIndex = this.suggestions.findIndex(
      ({ title, value }) =>
        input === title.toLowerCase() || input === value.toLowerCase(),
    );

    // fallback to current highlight
    if (newHighlightIndex < 0) {
      newHighlightIndex = this.suggestions.findIndex(
        ({ value }) => value === currentHighlight?.value,
      );
    }

    this.moveSelect(newHighlightIndex < 0 ? 0 : newHighlightIndex);
