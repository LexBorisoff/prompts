...
const { clear, style, wrap, figures } = require('../util');
/**
...
class AutocompleteMultiselectPrompt extends MultiselectPrompt {
  ...
  updateFilteredOptions() {
    ...
    this.filteredOptions = this.value.filter(
      ({ title, value }) =>
        input == null ||
        input === '' ||
        (typeof title === 'string' && title.toLowerCase().includes(input)) ||
        (typeof value === 'string' && value.toLowerCase().includes(input)),
    );

    // match input exactly
    let newHighlightIndex = this.filteredOptions.findIndex(
      ({ title, value }) => title === input || value === input,
    );

    // fallback to current highlight
    if (newHighlightIndex < 0) {
      newHighlightIndex = this.filteredOptions.findIndex(
  ...
  renderCurrentInput() {
    return `${this.input || color.gray(this.hint)}\n`;
  }
  ...
  renderDoneOrInstructions() {
    ...
    const output = [this.renderCurrentInput()];
    if (this.instructions === undefined || this.instructions) {
      output.push(this.renderInstructions());
    }
  ...
  render() {
    ...
    prompt += this.renderOptions(this.filteredOptions, false);
